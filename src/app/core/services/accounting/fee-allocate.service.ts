...
import { AllocateFees } from '../../classes/Accounting/feeMaintainance/allocate-fees';
...
//   }
  ...
  feeAllocate(_allocatedFees: AllocateFees): Observable<AllocateFees> {
    ...
    return this._http.post(`${this._feeAllocationUrl}`, body, this._utils.makeOptions(this._headers)).pipe(
      map((res: Response) => res.json()),
      tap(
...
//   }
  ...
  afterRequest(): void {
    this._utils.stop_progress();
    this.toastr.success('Done','Fee Allocated For This Category',{timeOut: 3000});
